...
def receive_messages():
  ...
  credentials = service_account.Credentials.from_service_account_file('/home/matt_cheung/.credentials/keys.json', scopes=scopes)
  #credentials = credentials.with_scopes(scopes=scopes)
  chat = build('chat', 'v1', credentials=credentials)

  subscription_id ='clara-gpt-sub' 
  ...
  def callback(message):
    logging.info('Received message: %s', message.data.decode('utf8'))
    ...
    temp = format_response(event)
    #temp = {'text': 'I got this'}
...
def format_response(event):
  ...
  match event_type:
    ...
    case 'MESSAGE':
      ...
      text = chatbot_g(event['message']['text'])
